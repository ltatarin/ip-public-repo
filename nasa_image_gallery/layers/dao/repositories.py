...
def saveFavourite(image):
    try:
        fav = Favourite.objects.create(title=image.title, description=image.description, comment=image.comment, image_url=image.image_url, date=image.date, user=image.user)
        return fav
